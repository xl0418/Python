...
def single_trait_sim(par):
    sim = traitsim(h = 1, num_iteration=1,num_species=10,gamma1=par[0],gamma_K2=par[0],a = par[1],r = 1,theta = 0,K = 5000
                   , mean_trait=0,dev_trait=20,mean_pop=50,dev_pop=20, num_time=2000)
    return sim
...
def ABC_acceptance(par,delta,obs):
...
def MCMC_ABC(startvalue, iterations,delta,obs):
    MCMCchain = np.zeros(shape=(iterations+1,2))
    MCMCchain[0,] = startvalue
    par_jump = np.empty(2)
    for i in range(iterations):
        par_jump[0] = np.random.normal(loc=MCMCchain[i,0], scale= 1 )
        par_jump[1] = np.random.normal(loc=MCMCchain[i,1], scale= 1 )
        if (ABC_acceptance(par_jump,delta = delta, obs = obs)):
