...
for i in range(evo_time):
    ...
    if (i+1) in speciate_time:
        ...
        trait_BH[i+1,daughter_index[spe_event_index-1]-1] = trait_BH[i+1,parent_index[spe_event_index-1]-1] + \
                                                            np.random.normal(0, 0.01, 1)
        population_BH[i + 1, daughter_index[spe_event_index-1]-1] =1/2 * population_BH[i + 1,
                                                                                       parent_index[spe_event_index-1]-1]
        population_BH[i + 1, parent_index[spe_event_index-1]-1] = 1 / 2 * population_BH[i + 1,
                                                                                        parent_index[spe_event_index-1]-1]
        trait_RI[i + 1, daughter_index[spe_event_index-1]-1] = trait_RI[i + 1,
                                                        parent_index[spe_event_index-1]-1] + np.random.normal(0, 0.01, 1)
        population_RI[i + 1, daughter_index[spe_event_index-1]-1] = 1 / 2 * \
                                                                population_RI[i + 1, parent_index[spe_event_index-1]-1]
        population_RI[i + 1, parent_index[spe_event_index-1]-1] = 1 / 2 * \
                                                                  population_RI[i + 1, parent_index[spe_event_index-1]-1]

    if (i + 1) in extinct_time:
        extinct_species = int(np.where(extinct_time == (i+1))[0])
        trait_BH[i+1, extinct_species] = None
        population_BH[i+1, extinct_species] = 0
        trait_RI[i + 1, extinct_species] = None
        population_RI[i + 1, extinct_species] = 0


trait_BH[np.where(trait_BH == 0)[0],np.where(trait_BH == 0)[1]] = None
trait_RI[np.where(trait_RI == 0)[0],np.where(trait_RI == 0)[1]] = None
...
plt.subplot(2, 4, 3)
sns.distplot(trait_BH[evo_time,:], hist=False, rug=True)

plt.subplot(2, 4, 4)
sns.distplot(population_BH[evo_time,:], hist=False, rug=True);
...
plt.subplot(2, 4, 7)
sns.distplot(trait_RI[evo_time,:], hist=False, rug=True)

plt.subplot(2, 4, 8)
sns.distplot(population_RI[evo_time,:], hist=False, rug=True);
