...
theta = 10   # optimum of natural selection
...
for i in range(num_time):
    ...
    trait_N1[i+1] = trait_N1[i] + 2*gamma * (theta - trait_N1[i]) * Gamma_N1 * (1 -np.exp(Gamma_N1) * Beta_N1/(K+(np.exp(Gamma_N1)-1) * Beta_N1))\
...
plt.subplot(2, 2, 4)
sns.distplot(population_N1[num_time,:], hist=False, rug=True);
