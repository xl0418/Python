...
for i in range(evo_time):
    ...
    Gamma_BH = ga_vector(gamma=0, theta=theta, zi=trait_BH[i,index_existing_species], r=r)
    ...
    Sigma_BH = sigma(a=a, zi=trait_BH[i,index_existing_species], zj=trait_BH[i,index_existing_species],
                     nj=population_BH[i,index_existing_species])
    trait_BH[i+1,index_existing_species] = trait_BH[i,index_existing_species] + 2 * (theta - trait_BH[i,index_existing_species]) \
                                           * Gamma_BH * (gamma - (r * (gamma - gamma_K))* Beta_BH/K_BH) + Gamma_BH  * Sigma_BH / K_BH +\
    ...
    K_RI = Kd_vector(gamma_K=0, theta=theta, zi=trait_RI[i,index_existing_species], K=K)
...
def animate(i, speciate_time, extinction_time):
    i = (i+1)%(len(x)+1)
